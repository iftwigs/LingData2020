...
b. Indicate the interval where the median of this sample can lie. Explain your answer.
   ...
   between -5 and 0 because the median is what we can call the "centre" of the sample -- and
   if we write down all the sample's elements, place them into their intervals and start
   going from the sides to the centre we'll see that the middle of the sample lies between -5 and 0.
   
c. How the histogram will change if we add an element $7$ to the sample? Explain your answer.
   
   the fourth bar will become higher by one point because it signifies elements which fall
   between 5 and 7.
...
the first histogram has a larger variance because informally speaking, variance is how much
the values can differ from the mean, and it is visible that the first histogram's leftmost and rightmost points are farther from the mean than in the second one.
...
the first histogram has a larger variance for the same reasons mentioned above -- the
numbers on the Ox allow us to see that in the first histogram the values differ from the
mean more than in the second one.
...
more elements lie between 0 mark and 35 mark.
...
it is false because the highest bar (= the most students) is somewhat close to the mark 20 on Ox.
...
20 is closer to be the median of mistakes because it is visible that the biggest part of mistakes lies between 10 and 20, somewhat closer to 20.
...
i will not reject the null hypothesis because 0.000001 is infinitely small and thus statistically insignificant.
...
```{r}
df <- read_tsv('https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/poetry_last_in_lines.csv')
rel <- df[df$Decade=='1920s',] %>%
  select(-Decade)
rel <- df[df$Decade=='1920s',] %>%
  select(-Decade)
rel_examples <- nrow(rel) 
verbs <- nrow(rel[rel$UPoS=='VERB',])
```
...
```{r}
binom.test(verbs, rel_examples, p=0.17)
```
...
```
i've got p-value=0.2297 which is not enough to reject the null hypothesis, so we cannot say that the verbs in verses written in 1920s are used differently in the rhyming position than in general.
```
...
Interpret results. 
```
i've got p-value=0.05172 which is not small enough to tell with 95% confidence that the initial assumption is correct.
```
